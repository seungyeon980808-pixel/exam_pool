...
원문 도식이 검수 manifest에서 `representation: native_semantic` 또는 설명·핵심점
필드로 제공되면, 해설 문장 이미지로 삽입하지 말고 편집 가능한 표·도형 설명·수식
블록으로 재구성한다. 해당 도식에 raster `path`가 없다는 이유로 빈 이미지나 PDF
페이지 캡처를 만들면 안 된다. 반대로 실제 raster 자산을 사용할 때는 `allowed`와
허용 사유, SHA-256, 문항 소유권을 모두 기록하고 순수 그래프·기하·삽화 외에는
자동 FAIL한다. 의미 도식은 HWPX `BinData` 이미지 감사 대상이 아니며, 시각 QA에서는
원문 좌표·라벨·수치가 보존됐는지 별도 확인한다.
