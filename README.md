...
| 화면 | 하는 일 |
|---|---|
| **명제 은행** | 참인 명제를 성취기준·단원별로 축적. 거짓 변형(오답 재료)과 근거(교과서 원문)를 붙여 관리 |
| **문항 설계** | 명제를 골라 정답형·합답형 문항 조립. 합답형 선지는 프리셋 버튼으로 자동 생성 |
| **세트 관리** | 문항을 담아 드래그로 배열. 배점 합·난이도 분포·성취기준 커버리지를 실시간 확인 |
...
1. **근거 문서 탭** → 교과서·교육과정 PDF가 있는 폴더를 지정해 인덱싱 (한 번만)
2. **명제 은행 탭** → `＋ 명제 등록`으로 참인 명제를 넣고, `상세`에서 근거 검색 → 드래그로 근거를 붙인다
3. 참 명제에 **거짓 변형**을 추가해두면 오답 재료가 쌓인다
4. **문항 설계 탭** → 유형(정답형/합답형)을 고르고 명제를 가져와 조립 → `검토` → `문항 저장`
5. **세트 관리 탭** → 세트를 만들고 문항을 담아 배열 → `세트 검토` → `hwppalette 문법 복사`
...
app/
  main.py            FastAPI 진입점
  db.py              SQLite 스키마 · 성취기준 seed 적재
  routes_bank.py     명제 · 거짓변형 · 근거 API
  routes_question.py 문항 · 세트 · 출력 API
  ...
  seed/standards.json  2022 개정 중학교 과학 성취기준 87개
static/              화면 (나이스 디자인)
tools/               성취기준 추출 도구
