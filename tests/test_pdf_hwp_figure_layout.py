...
@pytest.mark.parametrize(
    ...
    [
        (92.53, routing.FigureLayout.TWO_SMALL, True),
        (92.55, routing.FigureLayout.TWO_LARGE, False),
    ],
)
def test_route_figure_uses_registered_cell_scale_boundary_for_small_pair(
    ...
    # Given: equal horizontal panels straddling the final14 cell's 70% contain-scale limit.
    split = total_width / 2
    source = source_artifact(tmp_path, SourceArtifactSpec(
    ...
    assert routed.layout_metadata.minimum_projected_scale >= 0.7
...
def test_route_figure_keeps_unlabeled_multi_object_material_as_one_captionless_composite(
    ...
    source = source_artifact(tmp_path, SourceArtifactSpec(
        ...
        captions=(), drawing_count=0, image_count=1, component_count=3,
    ))
...
def test_route_figure_promotes_medium_single_figure_to_large_slot(tmp_path: Path) -> None:
    source = source_artifact(tmp_path, SourceArtifactSpec(
        name="medium-single",
        panel_bboxes=((0.0, 0.0, 160.0, 90.0),),
...
def test_inquiry_process_labels_are_not_figure_panel_captions() -> None:
    from app.pdf_hwp_raster_caption_segmentation import expected_panel_labels, panel_label_token

    inquiry = (
        "[탐구 과정] (가) 종이를 고정한다. (나) 투명 용지를 올린다. "
        "(다) 이동시킨다. (라) 점을 찍는다. (마) 반복한다."
    )
    figure = "그림 (가), (나), (다)는 서로 다른 지질 구조를 나타낸 것이다."
    assert expected_panel_labels(inquiry) == ()
    assert expected_panel_labels(figure) == ("(가)", "(나)", "(다)")
    assert panel_label_token("(가) 포획", ("(가)", "(나)", "(다)")) == "(가)"
    assert panel_label_token("(가)에서", ("(가)", "(나)", "(다)")) is None


def test_palette_question_maps_three_panel_direct_to_registered_hapdap_caption_template() -> None:
    question = question_structure.palette_question(
        "자료", "옳은 것은?", "a.png,b.png,c.png", routing.FigureLayout.THREE_SMALL,
    )
