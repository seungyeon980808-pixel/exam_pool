...
## 0-B. 2026-08-04 추가된 도구 — 손으로 하던 계산을 대신한다

| 도구 | 대신해 주는 일 |
|---|---|
| **`fit_artboard`** | 아트보드를 그린 내용에 맞춘다. **내보내기 직전 항상 부른다.** set_artboard 는 '그릴 범위'만 대충 잡고, 최종 크기는 이 도구가 정한다 — 축 이름·한글 라벨 폭까지 실제로 재므로 잘림이 원천적으로 없어지고 빈 여백도 사라진다(여백이 넓으면 시험지에서 그림이 그만큼 작아진다). |
| **`add_field_region`** | 균일 자기장 영역: 파선 테두리 + ⊗/⊙ 격자 + **범례 문장 자동**. `avoid` 로 도선이 지나가는 자리를 비운다. |

**자기장 문항의 구도 원칙**: 자기장을 종이면에 **수직**(⊗/⊙)으로 두어야 도선과 힘을
둘 다 면 안의 화살표로 그릴 수 있다. 자기장을 면 안에 그리면 힘이 면 밖으로 나가
화살표로 표현할 수 없다 — 2026-08-03 전동기 도판에서 힘 화살표를 포기했던 원인이다.
...
| 물리량 기호가 안 보임 | `formula`의 내용을 `text`에 넣음 — 다른 타입은 전부 `text`라 헷갈린다 | **`source`**에 넣는다. (2026-08-03 이후로는 서버가 오류로 막는다) |
| 축 이름 끝이 잘림 | 축 이름은 평면 **바깥**(왼쪽 위 / 오른쪽 아래)에 놓인다 | 아트보드 폭을 평면보다 20~30mm 넉넉히 |
| 엉뚱한 창에 그려짐 | 5E를 여러 개 열어 두면 마지막에 연 창이 통로를 가져간다 | `app_status`로 창 주소 확인. 파이썬은 `client.py`의 `wait_for_app(require=…)` |
